...
public class FlyWheel extends SubsystemBase {
	...
	public FlyWheel() {
		...
		FlyWheelTab.add("kP", kP).getEntry().addListener(notification -> {
			flywheelMotor1.config_kP(0, notification.value.getDouble());
		}, EntryListenerFlags.kUpdate);
		FlyWheelTab.add("kI", kI).getEntry().addListener(notification -> {
			flywheelMotor1.config_kI(0, notification.value.getDouble());
		}, EntryListenerFlags.kUpdate);
		FlyWheelTab.add("kD", kD).getEntry().addListener(notification -> {
			flywheelMotor1.config_kD(0, notification.value.getDouble());
		}, EntryListenerFlags.kUpdate);
		FlyWheelTab.add("kF", kF).getEntry().addListener(notification -> {
			flywheelMotor1.config_kF(0, notification.value.getDouble());
		}, EntryListenerFlags.kUpdate);
